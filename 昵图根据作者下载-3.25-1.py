...
class Spider(object):
    ...
    def _black(self, filename):
        ...
        with open(filename, 'rb') as f:
            ...
            f.seek(0)  # 重新定位文件指针到开头，避免重新打开文件
            content = f.read().decode(encoding)  # 读取文件并使用正确的编码解码
            return {x.strip().strip('#') for x in content.splitlines()}
    ...
    async def get_content(self, url, ):  # 传入的是图片连接
        async with aiohttp.ClientSession(headers=self.headers) as session:
            ...
            filebh = userinfo + '_' + filebh  # --------------------------加作者和内部编号
    ...
    async def _download_img(self, img, share_dict, mysem):
        ...
        if self.original:  # 下载原图
            ckpath = os.path.dirname(file_name_resize)
            if not os.path.exists(ckpath):
                os.makedirs(ckpath)
            with open(file_name_resize, 'wb') as f:
                f.write(content)
        else:
...
if __name__ == '__main__':
    ...
    para = dict(
        # keys_file=r'E:\下载\呢图\1.txt',  # txt
        keys_file=r'E:\下载\昵图\1.txt',  # txt
        # down_path=r'E:\下载\呢图\3.20更新',  # 图片存放基础路径,最终文件路径为这个目录加关键字
        down_path=r'E:\下载\昵图',  # 图片存放基础路径,最终文件路径为这个目录加关键字
        resize_path='略缩图',  # resize图片的子目录，默认 resize
        crop_path='裁剪图',  # crop图片的子目录，默认crop
        startpage=1,  # 开始页，默认1
        endpage=0,  # 结束页，默认1，如果此参数为0，那么会全部下载
        update_date='1023-04-14',  # 上传日期：不填写默认全部下载，日期为文本'yyyy-mm-dd'('2021-12-01')格式
        myinterval=60,  # 几秒钟检测一次是否还在下载，如果此参数为0，那么就不检测断网情况。默认60秒
        sem=100,  # 并发数，默认100
        delay=0.7,  # 爬取间隔数，防止被服务器踢掉，每爬一张图片间隔时间，默认0.5秒。

        black='不用下载编号.txt',
        original=True,  # 这个变量为True，那么就略缩图保存为原图，否则下载略缩图

    )
    with open(para.get('keys_file'), 'rb') as f:
        result = chardet.detect(f.read())
        encoding = result['encoding']
        encoding = 'ANSI'
        f.seek(0)  # 重新定位文件指针到开头，避免重新打开文件
        content = f.read().decode(encoding)  # 读取文件并使用正确的编码解码
        keys_list = map(str.strip, content.splitlines())
