...
class Spider(object):
    ...
    def __init__(self, down_path='', ):
        ...
        self.url = 'https://www.16pic.com/search/search?type=0&q={}'
    ...
    async def _get_img_links(self, page, q, session, get_totalpage=False):  # 获取图片连接
        ...
        try:
            async with session.get(url=url, ) as respone:
                ...
                path = '/html/body/div[3]/div[3]/div/div/a/@href'
                page_urls = [x if x.startswith(self.base_url) else f'{self.base_url}{x}' for x in el.xpath(path)]
                # print(page_urls)
                # return
