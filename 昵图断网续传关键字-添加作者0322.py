...
if __name__ == '__main__':
    ...
    para = dict(
        ...
        # keys_file=r'd:\download\1.txt',  # txt
        down_path=r'E:\下载\呢图\3.20更新',  # 图片存放基础路径,最终文件路径为这个目录加关键字
        # down_path=r'd:\download',  # 图片存放基础路径,最终文件路径为这个目录加关键字
        resize_path='略缩图',  # resize图片的子目录，默认 resize
        crop_path='裁剪图',  # crop图片的子目录，默认crop
        startpage=1,  # 开始页，默认1
        endpage=2,  # 结束页，默认1，如果此参数为0，那么会全部下载
        update_date='2023-03-06',  # 上传日期：不填写默认全部下载，日期为文本'yyyy-mm-dd'('2021-12-01')格式
